 ...
 */
    ...
    private fun copyDataBase() {
        ...
        while ({length = myInput.read(buffer);length }() > 0) {
            ...
            Log.d("takip", "kopyalıyor")
            Log.d("takip", buffer.toString())
        }
        if({length = myInput.read(buffer);length }() == 0){
            Log.d("takip", "veri yok")
        }
